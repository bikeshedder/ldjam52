...
impl AssetLoader for TiledLoader {
    fn load<'a>(
        ...
        Box::pin(async move {
            ...
            let tiled_dir = Path::new("assets").join(load_context.path());

            let map = loader
                .load_tmx_map_from(BufReader::new(bytes), tiled_dir)
                    ...
                    None => {
                        for (tile_id, tile) in tileset.tiles() {
                            if let Some(img) = &tile.image {
                                ...
                                let tile_path = img.source.strip_prefix("assets").unwrap().to_path_buf();
                                println!("tile_path={:?}", tile_path);
